...
# Date: 10/8/24
# Title: Lab 3 - Step 5 
# Description: In Computer Science, the producer–consumer problem is a classic multi- process synchronization
...
void producer(int write_fd) {
    char buff[BUFFER_SIZE] = "This is the Computer Science Producer-Consumer Problem";
    while(1) {
    printf("Producer Puts: %s\n", buff);
...
void consumer(int read_fd) {
    ...
    while (read(read_fd, buff, BUFFER_SIZE) > 0) printf("Consumer Removes: %s\n", buff);
}
